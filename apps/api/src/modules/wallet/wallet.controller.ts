import {
  Controller, Get, Post, Delete, Param, Body, UseGuards, ParseUUIDPipe, HttpCode, HttpStatus,
} from '@nestjs/common';
...
class RequestWithdrawalDto {
  @ApiProperty({ description: 'Amount in cents' }) @IsInt() @Min(1) amount: number;
...
export class WalletController {
  ...
  @Post('withdrawals')
  requestWithdrawal(@CurrentUser() user: JwtPayload, @Body() dto: RequestWithdrawalDto) {
    return this.walletService.requestWithdrawal(user.sub, dto);
  }

  @Delete('withdrawals/:id')
  @HttpCode(HttpStatus.NO_CONTENT)
  cancelWithdrawal(@Param('id', ParseUUIDPipe) id: string, @CurrentUser() user: JwtPayload) {
