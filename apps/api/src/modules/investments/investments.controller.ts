...
import { IsString, IsInt, Min, IsOptional, IsISO8601, IsEnum } from 'class-validator';
import { ApiProperty, ApiPropertyOptional } from '@nestjs/swagger';

class CreateInvestmentDto {
  @ApiProperty() @IsString() name: string;
  @ApiPropertyOptional() @IsString() @IsOptional() description?: string;
  @ApiProperty({ description: 'Amount in cents' }) @IsInt() @Min(1) investedAmount: number;
...
class UpdateInvestmentDto {
  ...
  @ApiPropertyOptional() @IsString() @IsOptional() description?: string;
  @ApiPropertyOptional({ enum: InvestmentStatus }) @IsEnum(InvestmentStatus) @IsOptional() status?: InvestmentStatus;
