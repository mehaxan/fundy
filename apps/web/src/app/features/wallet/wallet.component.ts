...
@Component({
      ...
      <div class="wallet-header">
        <mat-card class="balance-card">
          <mat-card-content>
            ...
            <p class="balance">{{ (wallet()!.balance - wallet()!.pendingDebits) / 100 | currency }}</p>
            <p class="muted">Total: {{ wallet()!.balance / 100 | currency }} · Pending out: {{ wallet()!.pendingDebits / 100 | currency }}</p>
          </mat-card-content>
          <mat-card-actions>
            <button mat-flat-button color="primary" (click)="openWithdrawDialog()">
          ...
          <mat-card-content>
            <form [formGroup]="withdrawForm" (ngSubmit)="submitWithdrawal()">
              <mat-form-field appearance="outline">
                <mat-label>Amount (in {{ wallet()!.balance | currency:'USD':'symbol':'1.0-0' }} — enter cents)</mat-label>
                <input matInput type="number" formControlName="amount" placeholder="e.g. 10000 = $100" />
              </mat-form-field>
              <mat-form-field appearance="outline">
              ...
              <div class="form-actions">
                <button mat-button type="button" (click)="showWithdrawForm.set(false)">Cancel</button>
          ...
          <table mat-table [dataSource]="transactions()" class="full-width">
            ...
            <ng-container matColumnDef="amount">
              ...
              <td mat-cell *matCellDef="let t" [class]="t.direction">
                {{ t.direction === 'credit' ? '+' : '−' }}{{ t.amount / 100 | currency }}
              </td>
            </ng-container>
            <ng-container matColumnDef="status">
              <th mat-header-cell *matHeaderCellDef>Status</th>
              <td mat-cell *matCellDef="let t"><mat-chip [class]="t.status">{{ t.status }}</mat-chip></td>
            </ng-container>
  ...
  styles: [`
    ...
    .balance-card { max-width: 400px; }
    .label { color: #757575; margin: 0 0 4px; }
    .balance { font-size: 2.5rem; font-weight: 700; margin: 0; }
    .muted { color: #757575; font-size: 0.875rem; margin: 4px 0 0; }
    .withdraw-form { max-width: 480px; margin-bottom: 24px; }
    mat-form-field { width: 100%; margin-bottom: 12px; }
...
export class WalletComponent implements OnInit {
  ...
  load() {
    ...
    this.http.get<WalletTransaction[]>('/api/wallet/transactions').subscribe((t) => this.transactions.set(t));
  }
  ...
  submitWithdrawal() {
    ...
    this.http.post('/api/wallet/withdrawals', { amount, notes }).subscribe(() => {
