...
@Component({
  ...
  standalone: true,
  imports: [MatTableModule, MatButtonModule, MatFormFieldModule, MatInputModule, MatIconModule, ReactiveFormsModule, CurrencyPipe],
  template: `
    <h2>All Wallets</h2>
    ...
    @if (selectedUserId()) {
      <div class="adjust-form">
        ...
        <form [formGroup]="adjustForm" (ngSubmit)="submitAdjust()">
          ...
          <button mat-flat-button color="primary" type="submit" [disabled]="adjustForm.invalid">Apply</button>
          <button mat-button type="button" (click)="selectedUserId.set(null)">Cancel</button>
        </form>
      </div>
    }
  `,
  styles: [`
    ...
    .adjust-form { margin-top: 24px; max-width: 480px; }
...
export class AdminWalletsComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get<any[]>('/api/admin/wallets').subscribe((w) => this.wallets.set(w));
  }
  ...
  submitAdjust() {
    ...
    this.http.post(`/api/admin/wallets/${this.selectedUserId()}/adjust`, { amount, notes }).subscribe(() => {
