    ...
    <table mat-table [dataSource]="withdrawals()" class="full-width">
      ...
      <ng-container matColumnDef="status">
        ...
        <td mat-cell *matCellDef="let w">
          <mat-chip>{{ w.status }}</mat-chip>
        </td>
      </ng-container>
      <ng-container matColumnDef="actions">
        <th mat-header-cell *matHeaderCellDef>Actions</th>
        <td mat-cell *matCellDef="let w">
          <button mat-stroked-button color="primary" (click)="confirm(w.id)">
            Confirm
          </button>
          &nbsp;
          <button mat-stroked-button color="warn" (click)="reject(w.id)">
            Reject
          </button>
        </td>
      </ng-container>
...
export class AdminWithdrawalsComponent implements OnInit {
  ...
  load() {
    this.http
      .get<any[]>("/api/admin/withdrawals")
      .subscribe((w) => this.withdrawals.set(w));
  }

  confirm(id: string) {
    this.http
      .patch(`/api/admin/withdrawals/${id}`, { status: "confirmed" })
      .subscribe(() => this.load());
  }

  reject(id: string) {
    this.http
      .patch(`/api/admin/withdrawals/${id}`, { status: "rejected" })
      .subscribe(() => this.load());
  }
}
